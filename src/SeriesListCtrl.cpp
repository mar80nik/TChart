...
BEGIN_MESSAGE_MAP(SeriesListCtrl, CListCtrl)
...
LRESULT SeriesListCtrl::OnDataUpdate(WPARAM wParam, LPARAM lParam )
{
	UpdateSeriesList();	
	return 0;
}
...
void SeriesListCtrl::SetState(int num, BYTE state)
{
	LV_ITEM lvi;	
	lvi.mask = LVIF_STATE;
	lvi.iItem = num;
...
BYTE SeriesListCtrl::GetState(int num)
{
...
void SeriesListCtrl::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
...
int SeriesListCtrl::HitTest(CPoint point)
{
	UINT f = LVHT_ONITEM; 
...
{
	int ret=0; 
	SeriesListCtrl* pListCtrl = (SeriesListCtrl*) lParamSort;
...
void SeriesListCtrl::OnChangeStatus()
{
}

void SeriesListCtrl::OnChangestatusActive() {ChangeStatus(SER_ACTIVE);}
void SeriesListCtrl::OnChangestatusInactive() {ChangeStatus(SER_INACTIVE);}
void SeriesListCtrl::OnChangestatusCalibrator() {ChangeStatus(SER_CALIBRATOR);}


void CreateCalibratedSeries() {};

void SeriesListCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	switch(nChar)
	{
...
HBRUSH SeriesListCtrl::OnCtlColor( CDC*, CWnd*, UINT )
{
	return bckg;
}

void SeriesListCtrl::OnMenuChangeStyle()
{
	POSITION pos=GetFirstSelectedItemPosition(); int n; BOOL show=FALSE;
